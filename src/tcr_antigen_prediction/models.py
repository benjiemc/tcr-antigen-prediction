...
class MasifPPISearch:
        ...
        with tf.Graph().as_default() as g:  # pylint: disable = not-context-manager
            ...
            with tf.device(idx_gpu):
                ...
                for i in range(self.n_feat):
                    self.mu_rho.append(tf.Variable(mu_rho_initial, name=f'mu_rho_{i}'))  # 1, n_gauss
                    self.mu_theta.append(tf.Variable(mu_theta_initial, name=f'mu_theta_{i}'))  # 1, n_gauss

                    self.sigma_rho.append(
                        tf.Variable(np.ones_like(mu_rho_initial) * self.sigma_rho_init, name=f'sigma_rho_{i}')
                    )  # 1, n_gauss

                    self.sigma_theta.append(
                        tf.Variable((np.ones_like(mu_theta_initial) * self.sigma_theta_init), name=f'sigma_theta_{i}')
                    )  # 1, n_gauss

                self.keep_prob = tf.placeholder(tf.float32)

                # **Features for binder should be flipped before feeding to the NN.
                self.rho_coords = tf.placeholder(tf.float32, shape=[None, None, 1])    # batch_size, n_vertices, 1
                self.theta_coords = tf.placeholder(tf.float32, shape=[None, None, 1])  # batch_size, n_vertices, 1
                self.input_feat = tf.placeholder(tf.float32,                           # batch_size, n_vertices, n_feat
                                                 shape=[None, None, self.n_feat])
                self.mask = tf.placeholder(tf.float32, shape=[None, None, 1])          # batch_size, n_vertices, 1
                ...
                b_conv = []
                for i in range(self.n_feat):
                    b_conv.append(tf.Variable(tf.zeros([self.n_thetas * self.n_rhos]), name=f'b_conv_{i}'))
                ...
                for i in range(self.n_feat):
                    ...
                    desc = self.inference(my_input_feat,
                                          self.rho_coords,
                                          self.theta_coords,
                                          self.mask,
                                          w_mat_conv,
                ...
                self.global_desc = tf.reshape(self.global_desc, [-1, self.n_thetas * self.n_rhos * self.n_feat])

                # Refine global_desc with a FC layer.
                # batch_size, n_thetas
                self.global_desc = tf.contrib.layers.fully_connected(self.global_desc,
    ...
    def inference(self,
        ...
        for k in range(self.n_rotations):
            ...
            rho_coords_ = tf.exp(-tf.square(rho_coords_ - mu_rho) / (tf.square(sigma_rho) + eps))
            thetas_coords_ = tf.exp(-tf.square(thetas_coords_ - mu_theta) / (tf.square(sigma_theta) + eps))

            gauss_activations = tf.multiply(rho_coords_, thetas_coords_)  # batch_size * n_vertices, n_gauss
            gauss_activations = tf.reshape(gauss_activations,             # batch_size, n_vertices, n_gauss
            ...
            if mean_gauss_activation:
                ...
                gauss_activations /= tf.reduce_sum(gauss_activations, 1, keep_dims=True) + eps

            gauss_activations = tf.expand_dims(gauss_activations, 2)  # batch_size, n_vertices, 1, n_gauss,

            input_feat_ = tf.expand_dims(input_feat, 3)  # batch_size, n_vertices, n_feat, 1

            gauss_desc = tf.multiply(gauss_activations, input_feat_)  # batch_size, n_vertices, n_feat, n_gauss,
            gauss_desc = tf.reduce_sum(gauss_desc, 1)                 # batch_size, n_feat, n_gauss,
            gauss_desc = tf.reshape(gauss_desc,                       # batch_size, 80
