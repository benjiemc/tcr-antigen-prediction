...
def screen_chain(structure: Structure.Structure,
    ...
    missing_residues_on_chain = [[IUPACData.protein_letters_3to1[entity['residue_name'].title()],
                                  ...
                                  entity['residue_insert_code'], 'missing'] for entity in missing_residues
                                 if entity['chain_id'] == chain_id]

    missing_atoms_on_chain = [[IUPACData.protein_letters_3to1[entity['residue_name'].title()],
                               entity['residue_seq_id'],
                               entity['residue_insert_code']] for entity in missing_atoms
                              if entity['chain_id'] == chain_id]

    raw_sequence_info = [[IUPACData.protein_letters_3to1[res.get_resname().title()],
                          res.id[1],
                          res.id[2].strip(),
                          'not-missing']
                         for res in raw_structure[0][chain_id].get_residues() if res.id[0] == ' ']
    imgt_sequence_info = [[IUPACData.protein_letters_3to1[res.get_resname().title()],
                           res.id[1],
                           res.id[2].rstrip()]
                          for res in structure[0][chain_id].get_residues() if res.id[0] == ' ']
...
def screen_pmhc_abd(structure: Structure.Structure,
                    raw_structure: Structure.Structure,
    ...
    '''
    if antigen_chain_id in (set([res['chain_id'] for res in missing_residues])
                            | set(res['chain_id'] for res in missing_atoms)):
        return False
    ...
    if mhc_type == 'MH1':
        ...
        raise ValueError('Invalid MHC type, must be MH1 or MH2')
