...
Test training model with contact probabilities.
  $ python -m tcr_antigen_prediction.models.apps.train_tcr_struct_map \
  > --seed 123 \
  > --batch-size 1 \
  > --num-epochs 1 \
  > --eval-interval 1 \
  > -o model-contact-probabilities \
  > --contact-maps $TESTDIR/data/contact_maps.h5 \
  > --contact-probabilities $TESTDIR/data/contact_probabilities.txt \
  > $TESTDIR/data/test.h5
  *: UndefinedMetricWarning: Only one class is present in y_true. ROC AUC score is not defined in that case. (glob)
    warnings.warn(
  *: UndefinedMetricWarning: Only one class is present in y_true. ROC AUC score is not defined in that case. (glob)
  ...
  *: UndefinedMetricWarning: Only one class is present in y_true. ROC AUC score is not defined in that case. (glob)
  ...
  >     np.loadtxt(os.path.join(os.environ['TESTDIR'], 'data', 'contact_probabilities.txt')),
  >     dtype=torch.float32,
  > )
  > ref_model1 = TCRStructMap(cdr_pmhc_contact_probabilities=contact_probabilities)
  > ref_model1.load_state_dict(torch.load(
  >     os.path.join(os.environ['TESTDIR'], 'reference', 'contact-probabilities', 'model_1.pt'),
  >     map_location=torch.device('cpu'),
  >     weights_only=True,
  > ))
  > ref_model1.eval()
  ...
  >     torch.tensor(mhcs, dtype=torch.float32),
  > )
  > test_model1 = TCRStructMap(cdr_pmhc_contact_probabilities=contact_probabilities)
  > test_model1.load_state_dict(torch.load(
  >     os.path.join('model-contact-probabilities', 'model_1.pt'),
  >     map_location=torch.device('cpu'),
  >     weights_only=True,
  > ))
  > test_model1.eval()
  ...
  > )
  > np.testing.assert_array_almost_equal(test_predictions1.detach().numpy(), ref_predictions1.detach().numpy())
  > ref_model2 = TCRStructMap(cdr_pmhc_contact_probabilities=contact_probabilities)
  > ref_model2.load_state_dict(torch.load(
  >     os.path.join(os.environ['TESTDIR'], 'reference', 'contact-probabilities', 'model_2.pt'),
  >     map_location=torch.device('cpu'),
  >     weights_only=True,
  > ))
  > ref_model2.eval()
  ...
  >     torch.tensor(mhcs, dtype=torch.float32),
  > )
  > test_model2 = TCRStructMap(cdr_pmhc_contact_probabilities=contact_probabilities)
  > test_model2.load_state_dict(torch.load(
  >     os.path.join('model-contact-probabilities', 'model_2.pt'),
  >     map_location=torch.device('cpu'),
  >     weights_only=True,
  > ))
  > test_model2.eval()
